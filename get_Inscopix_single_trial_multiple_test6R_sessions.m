...
if nargin==0
   ...
   % mouse_info.ID='304RL'; %
    ...
    %trial_info.folder_name='SCNVIP_test6R_blue';
...
switch ROI_method
    case 'suite2P'
        path='L:\';
    case 'Ins'
        path='D:\DATA_Glab\Inscopix\' ; %VIPGC148RR\VIPGC148RR_Sess5tiff\suite2p\plane0
end
...
switch exp
    case 'test_6R_multiple'
        cd ([ 'C:\Users\Anat\Documents\Inscopix_Projects\'  trial_info.folder_name '\VIPGC' mouse '_test6R' ])
    case 'Zstack'
        cd ([path 'VIPGC' mouse '\all_Zstack\sess' num2str(sess) '\'])
end
...
if ~exist(['VIPGC' mouse 'sess' num2str(sess) '_results.mat']) || strcmpi(button, 'No')
    % read the results produced by Inscopix 
            T = readtable(['VIPGC' mouse '_test6R_traces.csv']);
         ...
         case 'test_6R_multiple'
            ...
            if exist (['VIPGC_' mouse '_onset_multiple.mat'])
               ...
               Full_F=[];
                for i=1:N
                    figure
                    INDS=[data_slicer(i)+5:data_slicer(i)+ceil(size(all_sess_F,2)/4-10)];
                    %INDS=[data_slicer(i)+5:data_slicer(i)+ceil(size(all_sess_F,2)/4-10)-10];
                    % for 303L:
                    % INDS=[data_slicer(i)+5:data_slicer(i)+ceil(size(all_sess_F,2)/4-10)-125];
...
%                     end
                ...
                R_Full_F=reshape(Full_F,[size(Full_F,1),size(Full_F,2)/N,N]);
                x=int16(x);
...
%     
    for  i=1:N
        all_dF2{i}=[];
        all_t{i}=[];
        for ci=1:length(cell_ind)
            A=[];
            At=[];
            for hi=1:size(dF{i},2)
...
if check_seperation
    for i=1:N
        ...
        for ci=1:size(all_t{i},1)
            plot(all_t{i}(ci,:),all_dF{i}(ci,:)'); hold on
        end
    end
end
% 
...
for i=1:N
    ...
    save (['VIPGC' mouse 'test6R_results_sess' num2str(i) '_B' num2str(baseline_method)],'results');
end
